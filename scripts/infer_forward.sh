...
out_images_root=data/demo_data/output/final_result/forward/${seq_name}
checkpoint_dir=${out_root}/${seq_name}_dual/forward/smplx/nn
current_epoch=
mkdir -p ${out_images_root}
min_epoch=54500
current_epoch=$(
    CHECKPOINT_DIR="${checkpoint_dir}" \
    REF_TAR_DIR="${out_root}/${seq_name}_dual/forward/ref_tar" \
    MIN_EPOCH="${min_epoch}" \
    python - <<'PY'
import os
