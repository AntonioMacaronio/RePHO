...
class InterMimic_All(InterMimic):
    ...
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        self.dataset_index = to_torch([int(data_path.split('/')[-1].split('_')[0][3:]) for data_path in self.motion_file_retarget], dtype=torch.long).to(self.device)
        ...
        self.object_id = to_torch([self.object_name.index(name) for name in object_name], dtype=torch.long).to(self.device)
