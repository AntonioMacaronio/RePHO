# from smpl_sim.smpllib.smpl_joint_names import SMPLH_MUJOCO_NAMES
from smpl_sim.smpllib.smpl_joint_names import SMPLH_MUJOCO_NAMES
...
index_to_part = {idx: part for part, idx_list in smpl_vert_segmentation.items() for idx in idx_list}
...
model_path = './body_models'  # 替换为实际路径
...
def detect_penetration_smplx_object_watertight(
        smplx_global_vertices,
        raw_mesh_vertices=None,
        raw_mesh_faces=None,
        object_global_vertices=None,
        depth_thresh=2e-3,
        fix_normals=True,
        voxel_pitch=0.005):
    """
    使用Trimesh的Signed Distance Field (SDF)计算SMPL-X顶点与封闭物体mesh的穿透深度。

    参数:
        smplx_global_vertices: (N,3) np.ndarray, 世界坐标下的SMPL-X顶点
        raw_mesh_vertices: (M,3) np.ndarray, 物体局部或全局顶点
        raw_mesh_faces: (F,3) np.ndarray, 物体面索引
        object_global_vertices: (M,3) np.ndarray 或 None, 若给定则使用全局顶点
        depth_thresh: float, 穿透深度阈值（米）
        fix_normals: bool, 是否尝试修复法向与封闭性
        voxel_pitch: float, SDF采样体素间距（越小越精确，越慢）

    返回:
        any_penetration: bool, 是否有穿透
        stats: dict, 统计信息
        per_vertex: dict, signed_distance/depth/inside_mask
    """
    # === 1. 构建mesh ===
    verts = object_global_vertices if object_global_vertices is not None else raw_mesh_vertices
    mesh = trimesh.Trimesh(vertices=verts, faces=raw_mesh_faces, process=False)
    ...
    if not mesh.is_watertight:
        print("[Warn] Mesh is not watertight; SDF 结果可能不可靠。")

    # === 2. 构建SDF（Trimesh内部用体素场近似） ===
    # pitch越小越精确；默认5mm
    sdf = trimesh.proximity.signed_distance(
        mesh, np.asarray(smplx_global_vertices, dtype=np.float64)
    )
    # 注：若Trimesh版本太旧没有该函数，可换为 trimesh.proximity.signed_distance(mesh, points)

    # === 3. 穿透分析 ===
    signed = np.asarray(sdf)
    depth = np.maximum(0.0, -signed)
    inside_mask = signed < 0

    any_pen = bool((depth > depth_thresh).any())
    max_depth = float(depth.max())
    mean_depth = float(depth[inside_mask].mean()) if inside_mask.any() else 0.0
    frac_inside = float(inside_mask.mean())
    kmax = int(depth.argmax())

    stats = dict(
        any_penetration=any_pen,
        num_inside=int(inside_mask.sum()),
        frac_inside=frac_inside,
        max_depth=max_depth,
        mean_depth_inside=mean_depth,
        deepest_vertex_index=kmax,
        deepest_vertex_world=smplx_global_vertices[kmax],
        depth_thresh_used=depth_thresh,
    ...
    return any_pen, stats, per_vertex



def detect_penetration_smplx_object(smplx_global_vertices,
                                    raw_mesh_vertices=None,
                                    raw_mesh_faces=None,
                                    object_global_vertices=None,
                                    depth_thresh=2e-3,
                                    fix_normals=True):
    """
    检测 SMPL-X 与物体 mesh 的穿透情况。
    
    参数:
        smplx_global_vertices: (N,3) np.ndarray, 世界坐标下的人体顶点
        raw_mesh_vertices: (M,3) np.ndarray, 物体原始mesh顶点（局部或全局）
        raw_mesh_faces: (F,3) np.ndarray, 物体面片索引
        object_global_vertices: (M,3) np.ndarray 或 None
            若提供，则直接使用该全局顶点，否则使用 raw_mesh_vertices。
        depth_thresh: float, 穿透深度阈值（米）
        fix_normals: bool, 是否修复法向方向
    
    返回:
        any_penetration: bool, 是否有穿透
        stats: dict, 统计信息
        per_vertex: dict, 每个人体顶点的 signed_distance / depth / inside_mask
    """
    # === 1. 构建物体mesh ===
    verts = object_global_vertices if object_global_vertices is not None else raw_mesh_vertices
    mesh = trimesh.Trimesh(vertices=verts, faces=raw_mesh_faces, process=False)
    ...
    pts = np.asarray(smplx_global_vertices, dtype=np.float64)  # (N,3)
    result = trimesh.proximity.closest_point(mesh, pts)
    if len(result) == 2:
        loc, face_idx = result
    elif len(result) == 3:
        loc, dist, face_idx = result
    elif len(result) == 4:
        loc, dist, face_idx, _ = result
    else:
        raise ValueError(f"Unexpected output from trimesh.proximity.closest_point: {len(result)} items")

    n = mesh.face_normals[face_idx]          # (N,3) 对应外法向
    v = pts - loc                            # 指向点的向量
    signed = np.einsum('ij,ij->i', v, n)     # 外正内负的近似SDF
    ...
    stats = dict(
        ...
        deepest_vertex_index=kmax,
        deepest_vertex_world=smplx_global_vertices[kmax],
        closest_point_on_mesh=loc[kmax],
        normal_at_deepest=n[kmax],
        depth_thresh_used=depth_thresh,
    )
    per_vertex = dict(
        signed_distance=signed,
...
def detect_penetration_smplx_object_knearest_normals(
        smplx_global_vertices,
        raw_mesh_vertices=None,
        raw_mesh_faces=None,
        object_global_vertices=None,
        ...
        k_nearest=3):
    """
    检测 SMPL-X 与物体 mesh 的穿透情况（使用最近k个三角面法向平均判定 inside/outside）
    兼容所有 trimesh 版本，无需 closest_point_naive(k=...)
    """
    ...
    # === 2. 构建KDTree（使用三角面中心） ===
    tri_centers = mesh.triangles_center  # (F,3)
    face_normals = mesh.face_normals     # (F,3)
    tree = cKDTree(tri_centers)

    pts = np.asarray(smplx_global_vertices, dtype=np.float64)

    # 查询最近的k个三角面
    dists, face_indices = tree.query(pts, k=k_nearest)  # (N,k)
    if k_nearest == 1:
        face_indices = face_indices[:, None]
        dists = dists[:, None]

    # === 3. 平均法向 ===
    normals = face_normals[face_indices]         # (N,k,3)
    avg_normals = np.mean(normals, axis=1)
    avg_normals /= np.linalg.norm(avg_normals, axis=1, keepdims=True) + 1e-8

    # 最近三角面（第一个）
    face_idx_nearest = face_indices[:, 0]
    tri_nearest = mesh.triangles[face_idx_nearest]    # (N,3,3)
    # 对每个点找到最近点（投影到最近三角面）
    from trimesh.proximity import closest_point
    # 最近点（只用一个三角面即可）
    result = trimesh.proximity.closest_point(mesh, pts)
    if len(result) == 2:
        locs, face_idx = result
    elif len(result) == 3:
        locs, dists, face_idx = result
    elif len(result) == 4:
        locs, dists, face_idx, _ = result
    else:
        raise ValueError(f"Unexpected output from trimesh.proximity.closest_point: {len(result)} items")
    ...
    # === 4. 计算signed distance ===
    v = pts - locs
    signed = np.einsum('ij,ij->i', v, avg_normals)   # 外正内负
    depth = np.maximum(0.0, -signed)
    inside_mask = signed < 0
    close_enough_mask = np.linalg.norm(v, axis=1) < 0.1
    inside_mask = np.logical_and(inside_mask, close_enough_mask)

    #get the segment part of the inside points using index_to_part.get
    # inside_parts = [index_to_part.get(i, 'Unknown') for i in np.where(inside_mask)[0]]
    #get the inside points belong to 'leftHandIndex1' according to smpl_vert_segmentation, not hand indices
    left_hand_index1_indices = smpl_vert_segmentation['leftHandIndex1']
    inside_left_hand_index1 = [i for i in np.where(inside_mask)[0] if i in left_hand_index1_indices]
    left_hand_penetration = (len(inside_left_hand_index1)/len(left_hand_index1_indices))>0.7
    right_hand_index1_indices = smpl_vert_segmentation['rightHandIndex1']
    inside_right_hand_index1 = [i for i in np.where(inside_mask)[0] if i in right_hand_index1_indices]
    right_hand_penetration = (len(inside_right_hand_index1)/len(right_hand_index1_indices))>0.7

    left_hand_index2_indices = smpl_vert_segmentation['leftHand']
    inside_left_hand_index2 = [i for i in np.where(inside_mask)[0] if i in left_hand_index2_indices]
    left_hand_penetration2 = (len(inside_left_hand_index2)/len(left_hand_index2_indices))>0.5
    right_hand_index2_indices = smpl_vert_segmentation['rightHand']
    inside_right_hand_index2 = [i for i in np.where(inside_mask)[0] if i in right_hand_index2_indices]
    right_hand_penetration2 = (len(inside_right_hand_index2)/len(right_hand_index2_indices))>0.5

    hand_penetration = np.logical_or(left_hand_penetration, right_hand_penetration)
    hand_penetration2 = np.logical_or(left_hand_penetration2, right_hand_penetration2)
    hand_penetration = np.logical_or(hand_penetration, hand_penetration2)
    ...
    any_pen = np.logical_or(hand_penetration, depth > depth_thresh).any()
    ...
    stats = dict(
        ...
        deepest_vertex_index=kmax,
        deepest_vertex_world=smplx_global_vertices[kmax],
        closest_point_on_mesh=locs[kmax],
        normal_at_deepest=avg_normals[kmax],
        depth_thresh_used=depth_thresh,
        k_nearest_used=k_nearest,
        inside_mask=inside_mask,
        left_hand_penetration=left_hand_penetration,
        right_hand_penetration=right_hand_penetration,
    )
...
def load_obj_vertices(obj_file):
    ...
    points, face_indices = trimesh.sample.sample_surface_even(mesh, count=5000)
    return points


def process_one_t(args):
    smplx_vertices_t, obj_vertices_t, obj_faces = args
    return detect_penetration_smplx_object_knearest_normals(
        smplx_global_vertices=smplx_vertices_t,
        object_global_vertices=obj_vertices_t,
        raw_mesh_faces=obj_faces,
        depth_thresh=5e-2,
        fix_normals=True
    )
...
def get_penetration(human_path, obj_path, obj_mesh):
    ...
    if betas.shape[0] == 10:
        betas = np.concatenate([betas, np.zeros(6)])
    betas_tensor = torch.tensor(betas, dtype=torch.float32).unsqueeze(0).expand(pose_tensor.shape[0], -1)  # (1, 16) -> (N, 16)
    ...
    # Read obj file vertices using trimesh instead of manual parsing
    ...
    obj_verts = obj_verts.unsqueeze(0) # Add batch dimension
    obj_trans = torch.from_numpy(obj_trans).float()

    obj_angles = Rotation.from_rotvec(obj_angles).as_matrix()
    rot_matrices = torch.from_numpy(obj_angles).float()


    # Apply rotation and translation to object vertices
    obj_verts_all = torch.bmm(obj_verts.expand(rot_matrices.shape[0], -1, -1), rot_matrices.transpose(1,2))
    obj_verts_all = obj_verts_all + obj_trans.unsqueeze(1)

    obj_verts_all = obj_verts_all.detach().cpu().numpy()


    args_list = [(vertices[t], obj_verts_all[t], obj_faces) for t in range(obj_trans.shape[0])]

    with Pool(processes=2) as pool:
        results = list(tqdm(pool.imap(process_one_t, args_list), total=len(args_list)))


    any_pens, stats_all, per_vertex_all = zip(*results)
    for t in range(obj_trans.shape[0]):
        print(t, stats_all[t]['any_penetration'], stats_all[t]['max_depth'], stats_all[t]['mean_depth_inside'], stats_all[t]['deepest_vertex_index'],'belong to',index_to_part.get(stats_all[t]['deepest_vertex_index'], 'Unknown'), stats_all[t]['left_hand_penetration'], stats_all[t]['right_hand_penetration'])

    is_penetration = np.array(any_pens)
